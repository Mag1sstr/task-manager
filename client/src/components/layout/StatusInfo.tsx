...
function StatusInfo() {
  ...
  const STATUSED = (data ?? []).reduce<Record<StatusType, number>>(
    (acc, el) => {
  ...
  return (
    <ul className="flex gap-2.5 mb-15">
      {Object.entries(STATUSED).map(([key, v]) => {
        ...
        return (
          <StatusItem
            onClick={() => {
              if (isHas) {
                dispatch(deleteStatusType(key as StatusType));
              } else {
                dispatch(addStatusType(key as StatusType));
              }
            }}
            count={v}
      ...
      <StatusItem type={"cancelled"} count={info.cancelled.length}>
        <svg
          width="58"
          height="58"
          viewBox="0 0 58 58"
